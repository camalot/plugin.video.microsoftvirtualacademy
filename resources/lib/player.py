    ...
    def getVideoInfo(self, content, name, year, imdb, tvdb):
        ...
        try:
            if self.content == 'movie':
                ...
                self.tvshowtitle.translate(None, '\/:*?"<>|'), self.year, int(self.season), int(self.episode))
        except:
            pass
        ...
        try:
            if control.setting('resume_playback') == 'true':
                ...
                yes = control.yesnoDialog(
                    ...
                    self.name, control.lang(30463).encode('utf-8'), control.lang(30462).encode('utf-8'))
    ...
    def getLibraryMeta(self):
        try:
            if self.content == 'movie':
                ...
                meta = {'title': meta['title'], 'originaltitle': meta['originaltitle'], 'year': meta['year'],
                        'genre': meta['genre'], 'studio': meta['studio'], 'country': meta['country'],
                        'duration': meta['runtime'], 'rating': meta['rating'], 'votes': meta['votes'],
                        'mpaa': meta['mpaa'], 'director': meta['director'], 'writer': meta['writer'],
                        'plot': meta['plot'], 'plotoutline': meta['plotoutline'], 'tagline': meta['tagline']}
                ...
                meta = {'title': meta['title'], 'season': meta['season'], 'episode': meta['episode'],
                        'tvshowtitle': meta['showtitle'], 'premiered': meta['firstaired'], 'duration': meta['runtime'],
                        'rating': meta['rating'], 'director': meta['director'], 'writer': meta['writer'],
                        'plot': meta['plot']}

                poster = control.jsonrpc(
                    '{"jsonrpc": "2.0", "method": "VideoLibrary.GetTVShows", "params": {"filter": {"field": "title", "operator": "is", "value": "%s"}, "properties": ["thumbnail"]}, "id": 1}' % showtitle)
